...
public class ClientHandler implements Runnable {
    ...
    private String handleRequest(String request) {
        ...
        switch (command) {
            case "signin":
                if (params.length >= 2) {
                    return Mediator.signIn(params[0], params[1]) ? "Login bem-sucedido" : "Falha no login";
                } else {
                    return "Parâmetros inválidos para login";
                }
            case "signup":
                if (params.length >= 3) {
                    return Mediator.signUp(params[0], params[1], params[2]) ? "Cadastro bem-sucedido" : "Falha no cadastro";
                } else {
                    return "Parâmetros inválidos para cadastro";
                }
            case "sendMessage":
                if (params.length >= 3) {
                    return Mediator.sendMessage(params[0], params[1], params[2]) ? "Mensagem enviada" : "Falha ao enviar mensagem";
                } else {
            ...
            case "findEmail":
                ...
                if (params.length >= 1) {
                    User user = Mediator.findByEmail(params[0]);
